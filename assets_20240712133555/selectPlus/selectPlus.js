...
function applySelectedOptions(id_select, by_click){
  ...
  if(select.onchange && by_click === true){
    select.onchange();
  }
...
function selectOption(option){
  ...
  applySelectedOptions(selectPlus.dataset.to, true);
  delay(selectPlus);
}
...
// auxiliares
function applyOption(option, select_id){
  const option_HTML = `
    <div class="select-options fixed" data-value="${option.value}" onclick="selectOption(this)">${option.text}</div>
  `;

  const select_option_list = document.querySelector(`[data-to="${select_id}"] .select-options-list`);

  const exists_option = document.querySelector(`[data-to="${select_id}"] .select-options-list [data-value="${option.value}"]`);

  
  if(!exists_option && select_option_list){
    select_option_list.innerHTML += option_HTML;
  }
}

function removeOption(option, select_id){
  const exists_option = document.querySelector(`[data-to="${select_id}"] .select-options-list [data-value="${option}"]`);

  
  if(exists_option){
    exists_option.remove();
  }
}