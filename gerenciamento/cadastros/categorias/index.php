<?php
  ...
  // definições para importação dos módulos da página
    ...
    if(havePrivilegeToDo(@returnsConstData('ROUTE_INFO')['title'], 'C')){
      array_push($includes, returnsPathFromHost("gerenciamento", "cadastros", "categorias", "cadastro.php"));
    }

    if(havePrivilegeToDo(@returnsConstData('ROUTE_INFO')['title'], 'R')){
      array_push($includes, returnsPathFromHost("gerenciamento", "cadastros", "categorias", "listagem.php"));
    }

    if(havePrivilegeToDo(@returnsConstData('ROUTE_INFO')['title'], 'U')){
      array_push($includes, returnsPathFromHost("gerenciamento", "cadastros", "categorias", "edicao.php"));
    }
...
use System\Controller\Categorias;
  ...
  $GLOBALS['item-sidebar'] = "categorias";
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
