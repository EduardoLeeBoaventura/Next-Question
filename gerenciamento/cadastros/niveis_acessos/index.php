<?php
  ...
  // definições para importação dos módulos da página
    ...
    if(havePrivilegeToDo(@returnsConstData('ROUTE_INFO')['title'], 'C')){
      array_push($includes, returnsPathFromHost("gerenciamento", "cadastros", "niveis_acessos", "cadastro.php"));
    }

    if(havePrivilegeToDo(@returnsConstData('ROUTE_INFO')['title'], 'R')){
      array_push($includes, returnsPathFromHost("gerenciamento", "cadastros", "niveis_acessos", "listagem.php"));
    }

    if(havePrivilegeToDo(@returnsConstData('ROUTE_INFO')['title'], 'U')){
      array_push($includes, returnsPathFromHost("gerenciamento", "cadastros", "niveis_acessos", "edicao.php"));
      array_push($includes, returnsPathFromHost("gerenciamento", "cadastros", "niveis_acessos", "vinculo_privilegios.php"));
    }
